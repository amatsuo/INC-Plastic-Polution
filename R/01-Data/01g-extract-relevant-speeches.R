...
chair_id_cand <- df |> count(speaker) |> arrange(-n) |> slice(1) |> pull(speaker)

chat <- chat_openai(
  model = "gpt-4o-mini",
  system_prompt = "You are a political scientist who is anlyzing the conference minutes of an international negotiantion.",
...
chair_cand_speech <- df |> 
  ...
  filter(duration <= 100 & duration >= 5) |> 
  sample_n(3) |> 
  pull(text) 
...
THe First thing you want to know is who is the chair of the meeting. Do think the following spkear is the chair of the meeting (The first word of your answer should be Yes or No)? You read sample of three speeches by the speaker below
...
# df |>  filter(speaker == 21) # Chair
# df |>  filter(speaker == 30) # this is a translator 
